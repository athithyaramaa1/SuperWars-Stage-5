...
class Superwar {
  ...
  calculateScore = () => {
    ...
    // Type your code here
    let jcore = this.score;

    const score = this.players.reduce((acc, elem) => {
      acc[elem.type] += elem.wins;
      return acc;
    }, { hero: 0, villain: 0 });
  ...
  checkWin = () => {
    ...
    // Type your code here
    const totalStrengthHero = this.totalStrength('hero');
    const totalStrengthVillain = this.totalStrength('villain');
    
    if (totalStrengthHero > totalStrengthVillain) {
      return 'hero';
    } else if (totalStrengthHero < totalStrengthVillain) {
      return 'villain';
    } else {
      return 'endure';
    }
